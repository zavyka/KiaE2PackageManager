...
# Helper function to create standard 'ar' archive using Python natively
create_ar_archive() {
    local OUT_FILE="$1"
    shift
    local IN_FILES=("$@")
...
# Helper function to list container members via Python natively
...
# Helper function to extract container via Python natively
...
# Helper function to extract inner archives supporting all compression algorithms
extract_sub_archive() {
...
# Function for Package Compatibility Scanner
scan_project_type() {
    local P_DIR="$1"
    local CURR_ARCH="$2"
...
# Function for Strict Package Validation Test (PURE PYTHON PARSER)
validate_package() {
    local PKG_PATH="$1"
    local PKG_TYPE="$2" # deb or ipk
    ...
    # 1. Structure Check via Native Python Parser
    ...
    # 2. Extract Container Members via Python
    ...
    # 3. Control Metadata Parsing & Validation
...
while true; do
    ...
    # ==============================================================================
    # SECTION 1: PLUGINS & SKINS (.deb & .ipk)
    # ==============================================================================
    if [[ "$SECTION_CHOICE" == "1" ]]; then
        ...
        # --- BUILD WITH AUTO SEARCH FOR PARENT FOLDERS CONTAINING USR ---
        if [[ "$PKG_ACTION" == "1" ]]; then
            ...
            # SEARCH FOR EXISTING CONTROL FILE IN DEBIAN OR CONTROL DIRS
            CONTROL_FILE=""
            ...
            # DETERMINE EXACT OUTPUT FILENAME
            ...
            echo -e "${Y}Cleaning up extra files (.pyc and .pyo) from the project source...${NC}"
        ...
        # --- UNPACK (.deb / .ipk) TWO-STAGE PERFECT EXTRACTION ENGINE ---
            ...
            # STAGE 1: EXTRACT CONTAINER MEMBERS TO TEMP WORK_DIR (HARDENED PYTHON PARSER)
            extract_ar_container_python "$TARGET_PKG" "$WORK_DIR"

            # FALLBACK TO AR/TAR IF PYTHON STAGE 1 DID NOT WORK
            ...
            # STAGE 2: PROCESS AND EXTRACT WORK_DIR MEMBERS TO OUT_P_DIR
            for f_item in "$WORK_DIR"/*; do
                ...
                if [[ "$c_item" == control.tar* ]]; then
                    ...
                    # EXACT COPY FOR ANY OTHER CONTAINED FILE
            ...
            # CLEANUP EMPTY DEBIAN FOLDER IF WAS CREATED BUT HAD NO CONTROL FILES INSIDE
    ...
    # ==============================================================================
    # SECTION 2: IMAGES & ARCHIVES (.nfi, .tar.xz, .tar.gz, etc.)
    # ==============================================================================
        ...
        # --- PACK / COMPRESS IMAGE FOLDER (WITH --numeric-owner & AUTO-CLEANUP) ---
            ...
            # CLEANUP EXTENSIONS TO AVOID REPEATED TAR IN FILENAME
            NAME_NO_EXT="${B_NAME%_extracted}"
            NAME_NO_TAR="${NAME_NO_EXT%.tar}"

            # PRE-PACK CLEANUP (REMOVES TEMP FILES & COMPILED PYTHON FILES)
            echo -e "${Y}Cleaning up temporary files (.pyc, .pyo, /tmp/*) before packing...${NC}"
            find "$COMP_DIR" -type f \( -name "*.pyc" -o -name "*.pyo" \) -delete 2>/dev/null
            if [ -d "$COMP_DIR/tmp" ]; then
                rm -rf "$COMP_DIR/tmp/"* 2>/dev/null
            fi

            # ENTER DIRECTORY TO PACK ROOTFS CONTENTS DIRECTLY WITHOUT PARENT WRAPPER
            cd "$COMP_DIR" || true
