...
- **دروستکردن و دووبارە دروستکردنی پاکێج** — دروستکردنی فایلەکانی `.deb` و `.ipk` لە پرۆژەکانی پڵاگین/سکینی خۆتان
...
- **کارکردن لەگەڵ ئەرشیف** — دروستکردنی ئەرشیفی فشراو بە فۆرماتە جیاوازەکان (`.zip`, `.tar`, `.tar.gz`, `.tar.xz`, `.tar.bz2`)
- **ناسینەوەی خۆکار** — ناسینەوەی خۆکاری مێعماری وەرگر (ARM, ARM64, MIPS, x86/x64)
- **هاوگونی جیهانی** — کاردەکات لەگەڵ Python 2.7+ و Python 3.x
...
|--------|------|-----------|
| `.deb` | پاکێجی Debian | وەرگرەکانی Dreambox / DreamOS |
| `.ipk` | پاکێجی IPK | وەرگرەکانی OpenATV / OpenPLi / OE2.0 |
| `.tar.xz` | ئەرشیفی فشراوی tar | فیرموێر/نەرمەواڵە گشتییەکان |
| `.tar.gz` | tar فشراو بە gzip | فیرموێر/نەرمەواڵە گشتییەکان |
| `.tar.bz2` | tar فشراو بە bzip2 | فیرموێر/نەرمەواڵە گشتییەکان |
| `.nfi` | وێنەی فیرموێری NFI | نوێکردنەوەی فیرموێری Dreambox |
| `.zip` | ئەرشیفی ZIP | فشاردانی فایلە گشتییەکان |
...
|------|---------|-------------|
| **ڕێگای ١** | هەموو کەس | تەنها دەستگەیشتن بە SSH بۆ وەرگر |
...
#### ڕێگای ١: داگرتنی ڕاستەوخۆ لەسەر وەرگر (ئاسانترین)

> **باشترین بۆ:** ئەوانەی دەیانەوێت خێرا ئامرازەکە جێبەجێ بکەن
> **پێویستە:** دەستگەیشتن بە SSH بۆ وەرگر (Putty، Terminal و هتد)

تەنها بە SSH پەیوەندی بە وەرگرەوە بکەن و ئەم فەرمانانە جێبەجێ بکەن. ئامرازەکە ڕاستەوخۆ لە GitHub بۆ وەرگر دادەگیرێت:
...
> **باشترین بۆ:** بەکارهێنەرانی پێشکەوتوو کە ئاشنای Terminal ـن
> **پێویستە:** کۆمپیوتەر لەگەڵ Terminal + دەستگەیشتن بە SSH بۆ وەرگر

سەرەتا فایلەکە لەسەر کۆمپیوتەر دابگرن، پاشان بە فەرمانی `scp` بۆ وەرگر بیگوازنەوە:

```bash
# هەنگاوی ١: ئەمە لەسەر کۆمپیوتەرەکەتان جێبەجێ بکەن (نەک لەسەر وەرگر)
scp KiaE2PackageManager root@<receiver_ip>:/tmp/

# هەنگاوی ٢: بە وەرگر پەیوەست بن و جێبەجێی بکەن
ssh root@<receiver_ip>
chmod +x /tmp/KiaE2PackageManager
/tmp/KiaE2PackageManager
```
...
ئەگەر ئاشنای بە هێڵی فەرمان (Command Line) نین، لەم نەرمەواڵانە بەکاربهێنن بۆ گواستنەوەی فایل بە ڕاکێشان و دانان (Drag & Drop):

**هەنگاوی ١: گواستنەوەی فایل بۆ وەرگر**

لە هەر یەکێک لەم نەرمەواڵانە دەتوانن فایلەکەی `KiaE2PackageManager` بۆ بوخچەی `/tmp/` لەسەر وەرگر کۆپی بکەن:

| نەرمەواڵ | پرۆتۆکۆڵ | ڕێنمایی |
|----------|-----------|---------|
| **FileZilla** | FTP | پەیوەستبوون بە `<receiver_ip>:21` → چوون بۆ `/tmp/` → ڕاکێشان و دانانی فایل |
| **WinSCP** | SCP/SFTP | پەیوەستبوون بە `<receiver_ip>` (پۆرتی 22) → چوون بۆ `/tmp/` → ڕاکێشان و دانانی فایل |
...
**Putty** (یان هەر SSH Client ـێکی تر) بکەرەوە و بە وەرگر پەیوەست ببە:
...
2. هەڵبژاردەی `1` (دروستکردن / دووبارە دروستکردنی پاکێج) هەڵبژێرە.
3. فۆرماتی دەرچوون هەڵبژێرە:
   - `1` — `.deb` (بۆ Dreambox / DreamOS)
   - `2` — `.ipk` (بۆ OpenATV / OpenPLi / OE2.0)
   - `3` — هەردوو فۆرمات
4. ئامرازەکە بوخچەکانی پرۆژە لە `/tmp/` پشکنین دەکات.
...
> **⚠️ گرنگ:** فایلەکانی فیرموێر دوای دەرهێنان قەبارەیەکی زۆر گەورەیان دەبێت (چەندین گیگابایت). پێشنیار دەکرێت لەسەر کارتی Micro SD یان هارد دیسکی دەرەکی یان Flash Memory پاشەکەوتیان بکەن، بۆ ئەوەی بۆشایی ناوخۆیی وەرگر پڕ نەبێتەوە.

**دروستکردنی ئەرشیف:**

1. هەڵبژاردەی `2` لە پێڕستی سەرەکی هەڵبژێرە.
2. هەڵبژاردەی `2` (پێچانەوە / فشاردانی بوخچە) هەڵبژێرە.
3. بوخچەی مەبەست هەڵبژێرە.
4. فۆرماتی دەرچوون هەڵبژێرە.
...
| ناتوانرێت بە SSH پەیوەندی بکرێت | دڵنیابە کە ڕاژەکاری SSH لەسەر وەرگر چالاکە (دامەزراندن: `opkg install openssh`) |
